...
def calc_auc_pixel(logger, dataset, n_vid, save_path, video_root_path="VIDEO_ROOT_PATH"):
    ...
    precision, recall, threshold = precision_recall_curve(all_gt, all_pred)
    auc = auc(recall, precision)
    ap = average_precision_score(all_gt, all_pred)


    logger.info("Dataset {}: Overall Pixel AUC = {:.2f}%, Overall Pixel EER = {:.2f}%".format(dataset, auc*100, ap*100))

    plt.plot(recall, precision)
    plt.plot([0,1],[1,0],'--')
    plt.xlim(0,1.01)
    plt.ylim(0,1.01)
    plt.title('{0} AUC: {1:.3f}, EER: {2:.3f}'.format(dataset, auc, ap))
...
def reconstructPrediction(prediction, out_dir):
    import numpy as np
    from scipy.misc import imresize, imsave
    import os
    predicted_frames = imresize(np.expand_dims(prediction, 1), (num_of_frame, 1))
    file_size = predicted_frames.shape(0)
    t = predicted_frames.shape(1)
    for vol in range(file_size):
       for i in range(t):
           predicted_frames[vol + i, :, :, :] += prediction[vol][i]

    for i in range(file_size+t):
        image_path = os.path.join(out_dir, "pred{0}.png".format(i))
        imsave(image_path, predicted_frames[i])
...
def test(logger, dataset, t, job_uuid, epoch, val_loss, visualize_score=True, visualize_frame=False,
    ...
    os.makedirs(os.path.join(save_path, 'reconstruction'), exist_ok=True)
    ...
    for videoid in range(n_videos[dataset]):
        ...
        reconstructPrediction(res, filesize+t, os.path.join(save_path, 'reconstruction'))
        ...
        if False: #visualize_score:
            ...
            for j in range(filesize):
               vol_costs[j] = np.sum((np.squeeze(res[j])-np.squeeze(X_test[j]))**2)
            ...
            #raw_costs = np.zeros(filesize + t)
            #for vol in range(filesize):
            #    for i in range(t):
            #        errA = np.sqrt((res[count, i, :, :, :] - X_test[count, i, :, :, :])**2)
            #        # errA = errA * (errA > 0.2)
            #        pixel_costs[vol + i, :, :, :] += errA
            #    count += 1
            #for i in range(filesize+t):
            #    raw_costs[i] = np.sum(pixel_costs[i])

            #method3
            # raw_costs = np.zeros((filesize,))
            # for j in range(filesize):
            #     raw_costs[j] = 10*np.log10((np.max(res[j])**2)/(np.sum((np.squeeze(res[j])-np.squeeze(X_test[j]))**2)/res[j].size))
            # raw_costs = imresize(np.expand_dims(raw_costs, 1), (filesize + t, 1))
            # raw_costs = np.squeeze(raw_costs)
